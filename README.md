...
roma-matcher deploys through the `up-1.0.79.sh` self-update migration, which registers it on
GPU-capable devices during the autoupdater's own update — no manual per-device step.

1. **Merge** this PR to `master`.
2. **Cut the release:** `./release.sh 1.0.79`.
3. **Trigger the self-update on each target device** — the `:8080` UI "Update" on the
   `autoupdater` package, or `./autoupdater.sh update autoupdater`.
4. **Deploy roma-matcher:** `./autoupdater.sh update-all` (or `./autoupdater.sh update roma-matcher`).

**GPU gate:** registers only where Docker reports the `nvidia` runtime; other devices are skipped.

> **Detection caveat:** no background poll — a device sees a new tag only on AutoUpdater
> restart or `:8080` UI/API query; applying it is an explicit per-device trigger.
